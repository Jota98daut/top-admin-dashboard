...
const trendingUsers = [
  {username: "tegan", text: "World Peace Builder", backgroundColor: "aliceblue"},
  {username: "morgan", text: "Super Cool Project", backgroundColor: "#FFCA28"},
  {username: "kendall", text: "Life Changing App", backgroundColor: "#7C4DFF"},
  {username: "alex", text: "No Traffic Maker", backgroundColor: "#F48FB1"},
];

projects.forEach(createCard);
announcements.forEach(createAnnouncement);
trendingUsers.forEach(createTrending);
...
function createTrending(trendingUser) {
  /*
  ...
  trendingDiv.appendChild(profilePic);
  profilePic.classList.add('profile-pic');
  profilePic.style.backgroundColor = trendingUser.backgroundColor;

  const rightText = document.createElement('div');
  trendingDiv.appendChild(rightText);

  const username = document.createElement('span');
  rightText.appendChild(username);
  username.textContent = `@${trendingUser.username}`;

  rightText.appendChild(document.createElement('br'));

  const text = document.createElement('span');
  rightText.appendChild(text);
  text.textContent = trendingUser.text;
  text.style.color = "var(--grey-foreground, grey)";
}
